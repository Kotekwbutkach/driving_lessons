...
class PlotGenerator:
    ...
    def __init__(self,
        ...
        self.name = name
        self.start = start
        self.end = self.road.time_step if end is None else min(end, self.road.time_step)
    ...
    def plot_vehicle_data(self):
        ...
        for value_id, value_name in value_types:
            for vehicle_id in range(self.road.number_of_vehicles):
                plt.plot(list(range(self.start, self.end)),
                         self.road.road_data[self.start:self.end, vehicle_id][:, value_id],
                         ",",
